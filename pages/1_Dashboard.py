...
#
# live_data = yf.download(ticker, period="1mo")
#
# if not live_data.empty:
#     try:
#         # 🔥 Ensure we extract clean numeric values
#         close_prices = live_data["Close"]

#
#         # Convert to list of floats safely
#         close_list = close_prices.values.flatten()
#
#         live_price = float(close_list[-1])
#         prev_price = float(close_list[-2]) if len(close_list) > 1 else live_price
#
...
#             margin-top:10px;
#         ">
#             <h4 style="color:#9ca3af;">💰 Live Price</h4>
#             <h2 style="color:#22c55e; margin:5px 0;">
#                 ${live_price:.2f}
#             </h2>
...
#         st.sidebar.warning("⚠️ Live data error")
#
# # live_data = yf.download(ticker, period="1mo")
# #
# # if not live_data.empty:
# #     # 🔥 Convert to float properly
# #     close_prices = live_data['Close'].dropna()
# #
# #     live_price = float(close_prices.iloc[-1])
# #     prev_price = float(close_prices.iloc[-2]) if len(close_prices) > 1 else live_price
# #
# #     change = live_price - prev_price
# #
# #     st.sidebar.metric(
